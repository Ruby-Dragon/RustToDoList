...
pub const LIST_FILE_NAME: &str = ".storedtodolist";

//parse the arguments passed in
pub fn parse_args(args: &Vec<String>)
{
    //Current list opened
    let mut path_string: String = dirs::home_dir().expect("Failed to find home directory").to_str().expect("Unable to convert as string").to_owned() + "/" + LIST_FILE_NAME;
    let mut open_list: crate::list::TaskList;use crate::list::*;
    use crate::task::*;
    use crate::files::*;
    use std::path::Path;
    use dirs;
    ...
    pub fn parse_args(args: &Vec<String>)
    {
        ...
        if Path::new(&path_string).exists() {
            open_list = crate::files::read_list_from_file(&path_string);
        } else {
            let _file = std::fs::File::create(&path_string)
        ...
        if args.len() > 0 {
            ...
            if args[0] == "add" {
                if args.len() > 2 {
                    //make a new task
                    let new_task = crate::task::Task {
                        name: args[1].clone(),
                        is_complete: false,
                        date: chrono::NaiveDate::parse_from_str(&args[2], "%Y-%m-%d")
                            .unwrap(),
					};
            ...
            crate::files::write_list_to_file(open_list, &path_string);
        } else {
            //print the list and save it in the file
            print!("{}", open_list.to_string());
            crate::files::write_list_to_file(open_list, &path_string);
